...
print(head(data))  
  ...
  for (l in genes) {
    ...
    if (sum(data.l$y)==0) {
      ...
      tmp <- LB(data.l, sig = sig)  #logistic beta
    }
...
tester.set.HD.single <- function(data, sig = 0.05, skeleton = FALSE) {
  ...
  #1-3. LB
  tmp <- LB(data, sig = sig)  #logistic beta
...
# print("9. Wagner done")   
  ...
  if (FALSE) {# examples
    ...
    tester.set.HD.batch(data, n.sim=30)
  }


### 1~3. LB
LB <- function (data, sig = 0.05) {
  require(gamlss)
...
# print(head(data$y.prop))  
  ...
  bereg = try(gamlss(y.prop ~ phenotype + batch, nu.formula = ~ phenotype + batch,
                     family = BEZI(sigma.link = "identity"), data = data,
