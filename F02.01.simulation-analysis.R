...
pval.plot <- function(result.stat, i = 1, test = "LB.glob", k.index=1:30, j.index = 1:3, ylim=0:1, title = NULL) {
  
  test.full = test.name[test == test.name[,1],2]  # full name
  delta.description = delta[i,]
  param.k = apply(parameter1[k.index,-1], 1, function(x) paste0("(", paste(x, collapse=", "), ")"))
  ...
  if (i == 8) { # legend page
    ...
    grid:::textGrob(paste0("Test = ", test.full, "\n",
                           "\nColor = batch effect (red: large, blue: small, green: no)",
                            "\n\nParam = 1~9 (pi=50%), 2~18 (pi=90%), 19~27 (pi=95%)",
...
if (FALSE) {
  ...
  pval.plot(result.stat, i=8, test="LN")
  ggsave()
